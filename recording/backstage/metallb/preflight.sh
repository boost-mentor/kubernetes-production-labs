...
test "$(kubectl --context "$SELF_CONTEXT" get nodes --no-headers | awk '$2 == "Ready" {n++} END {print n+0}')" -eq 3
helm version --short
kubectl --context "$SELF_CONTEXT" auth can-i create namespace | grep -q '^yes$'
kubectl --context "$SELF_CONTEXT" -n kube-system get configmap kube-proxy -o yaml | grep -q 'strictARP: true'

node_ips=$(
  kubectl --context "$SELF_CONTEXT" get nodes -o jsonpath='{range .items[*]}{.status.addresses[?(@.type=="InternalIP")].address}{"\n"}{end}'
)
