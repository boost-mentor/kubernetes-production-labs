...
[ "$NR" -eq 0 ] && ok "все ноды Ready" || fail "$NR нод не Ready — сначала почини"

echo "→ 3. Системные поды здоровы"
BAD=$(kubectl get pods -n kube-system --no-headers | grep -Ev "Running|Completed" | wc -l | tr -d ' ')
[ "$BAD" -eq 0 ] && ok "kube-system в порядке" || fail "$BAD проблемных подов"
...
[ -n "$SINGLE" ] && { warn "деплойменты с 1 репликой — при drain лягут:"; echo "$SINGLE"; } || ok "одиночных реплик нет"
