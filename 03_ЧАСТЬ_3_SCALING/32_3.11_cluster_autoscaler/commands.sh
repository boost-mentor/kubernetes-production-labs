...
MANIFEST="$LAB_DIR/night_shift.yaml"
...
kubectl --context yc-managed -n traffic-lab patch deploy night-shift --type=json   -p='[{"op":"replace","path":"/spec/template/spec/containers/0/resources/requests/cpu","value":"1500m"}]'
kubectl --context yc-managed -n traffic-lab scale deploy night-shift --replicas=3
kubectl --context yc-managed -n traffic-lab get pods -l app=night-shift -w
# Одна реплика гарантированно Pending на двух nodes по 2 vCPU; Ctrl+C.
PENDING_POD=$(kubectl --context yc-managed -n traffic-lab get pod -l app=night-shift   --field-selector=status.phase=Pending -o jsonpath='{.items[0].metadata.name}')
...
kubectl --context yc-managed -n traffic-lab patch deploy night-shift --type=json   -p='[{"op":"replace","path":"/spec/template/spec/containers/0/resources/requests/cpu","value":"100m"}]'
